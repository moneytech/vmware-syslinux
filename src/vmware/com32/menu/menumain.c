...
static const char *edit_cmdline(const char *input, int top)
{
    ...
    int rows, cols;
    int dwlen, dwoff, max_dwlen; /* Display window length and offset into cmdline */
    ...
    max_dwlen = (rows - CMDLINE_ROW - 1)*cols - 6;
    dwlen = dwoff = -1;
    prev_len = 0;
	...
	if (redraw > 0) {
            int prev_dwoff = dwoff;

            /* Shrink the display window when possible */
            if (prev_len != len && len > dwlen) {
                dwlen = min(len, max_dwlen);
            }

            /* Move display window to the left if required */
            if (cursor < dwoff) {
                 dwoff = cursor;
            }
      
            /* Move display window to the right along with the cursor */
            if (dwoff < 0 || (cursor - dwoff + 1) > dwlen) {
                 dwoff = cursor - dwlen + 
                         (dwlen != len ? 1 : 0); /* There is no need to move if 
                                                    display window isn't being used */
            }

            // Debug
            //printf("\033[?25l\033[4;1Hc: %d, len: %d, dwlen: %d, dwoff: %d     \n", 
            //                                            cursor, len, dwlen, dwoff);

	    /* Redraw the command line */
            if (prev_dwoff != dwoff || prev_len != len) {
                printf("\033[?25l\033[%d;1H\1#9%s\2#10%s%s",
                       CMDLINE_ROW, (dwoff > 0) ? ".." : "> ", 
                       pad_line(cmdline + dwoff, 0, dwlen),
                       (dwoff + dwlen < len) ? "  \1#9..\2#10" : "\2#10    ");
            }
	    printf("\2#10\033[%d;3H%s\033[?25h",
     	           CMDLINE_ROW, pad_line(cmdline + dwoff, 0, cursor - dwoff));
	    prev_len = len;
	    redraw = 0;
	}
	...
	default:
	    if (key >= ' ' && key <= 0xFF && len < MAX_CMDLINE_LEN - 1) {
	        if (cursor == len && 
                    (dwoff + dwlen) <= len) { /* Use put_char if there is empty
                                                 space in the drawing window */
		    cmdline[len] = key;
		    cmdline[++len] = '\0';
		    cursor++;
                    dwlen = min(len, max_dwlen);
                    if (dwlen == max_dwlen) { /* Redraw on moving drawing window */
                        dwoff++;
                        redraw = 1;
                    } else {
		        putchar(key);
